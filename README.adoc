...
* Download the most up-to-date version of this book from this repository
  - https://github.com/pinggit/kubernetes-contrail-day-one/blob/master/day-one-containers-kubernetes-contrail-v1.pdf (version v1)
  - https://github.com/pinggit/kubernetes-contrail-day-one/blob/master/day-one-containers-kubernetes-contrail.adoc (v0.9, "source code" of the book content)
  - https://github.com/pinggit/kubernetes-contrail-day-one/blob/master/day-one-containers-kubernetes-contrail.html (v0.9, with a TOC panel on the right)
...
== suggestions/contributing/issues
...
this book currently with 3 formats. 
* pdf `day-one-containers-kubernetes-contrail.pdf`
* adoc `day-one-containers-kubernetes-contrail.adoc`
* html `day-one-containers-kubernetes-contrail.html`


This book was originally written in adoc format, which is github version
contrail friendly. it went through many changes via git commit/PR process.
however, some final modifications (mostly minor though) between authors and
editors is via an "offline" process only through word and PDF, and hence not
cought by github yet. in another word, those last changes are in PDF format
only and has not been merged into adoc/html format yet.

=== PDF book TOC

the TOC of book PDF (Containers_Kubernetes_Contrail.pdf) has some problem. for
example chapter 6 cuently shows:
