...
and heavily used in practice that, a very similar new object has been designed
with more powerful features introduced. The original RC object and its
abstraction has been split into 2 new objects, named `RS` and `deployment`.
let's stop exploring more RC features for now and move our focus to these 2
objects.

=== RS and deployment

a RS object is pretty much the same thing as a RC object, with just one major
exception - the looks of labels.
...
RC uses "Equality-based" selector while RS uses "set-based". functionally the
...
now compare the `kubectl describe` on the 2 objects:
...
again, most part of the output are the same, with only exception of selector
format. 
...
with a different selector format, now we'll check out `deployment` and explore
the features coming from it. Again let's look at the yaml file of a typical
deployment object:
...
You might be wondering why you need RS as one more layer sitting in between
deployment and pod, after all with RC it seems sufficient to keep a set of pods
running.

the reason is about pod update - the "rolling update" feature that is one of the
"more powerful feature" mentioned earlier. this is how it works:

when you update the pod with new software, the `deployment` object introduces a
new RS that will start from creating a new pod equiped with the new software
release in it. once this new pod is up and running, the original RS will be
"scaled down" by 1, making the total number of running pod remaining unchanged.
new RS will continue to scale up by one and original RS scales down by one. this
process repeats until number of pods in new RS reaches the original replica
number defined in the deployment, and that is the time when all of the original
RS's pods are gone. 
...
This whole process of creating a new RS, scaling up the new RS and scaling down
the old one simultaneously, is fully automated and taken care of by the
deployment object. in contrast, RC alone, without this additional higher layer
abstraction, is not able to coordinate this process.

deployment also has the ability to "record" the whole process, so in
case needed, you can display the update history, pause/resume the process, or
even "undo" the upgrade - pretty much similar as the junos `rollback` magic
command that you use everyday to quickly revert the changes you make to your
router. comparing with how much work it takes to prepare for the software
upgrade during maintenance window in the old days, how handy it is with this
feature!
