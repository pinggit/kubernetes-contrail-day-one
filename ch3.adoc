...
One important reason is that each LoadBalancer service requires its own load
...
provide features such as cookie-based session affinity and the like, which
services can’t.

an Ingress controller needs to be running in the cluster. Different Kubernetes
environments use different implementations of the controller

there are basically 3 types of ingress:

* Single Service Ingress
* Simple fanout Ingress
* Name based virtual hosting Ingress

we'll look at each ingress's yaml file examples. and we'll then use nginx as ingress
controller to demonstrate each type of ingress.
